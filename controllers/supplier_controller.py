...
class SupplierController:
    ...
    def register_routes(app, db):
        @app.route('/suppliers')
        def suppliers_page():
            return render_with_message('table_suppliers.html', suppliers=Supplier.read_all(db))
        ...
        def create_supplier():
            ...
            Supplier.create(db, delivery_price=delivery_price, company_name=form_data['company_name'])
        ...
        def edit_supplier(supplier_id):
            supplier = Supplier.query(db).get_or_404(supplier_id)
            if request.method == 'POST':
                ...
                Supplier.update(db, supplier_id, delivery_price=delivery_price, company_name=form_data['company_name'])
                return redirect(url_for('suppliers_page'))
            return render_template('edit_supplier.html', supplier=supplier)

        @app.route('/delete_supplier/<int:supplier_id>', methods=['POST'])
        def delete_supplier(supplier_id):
            Supplier.delete(db, supplier_id)
            return redirect(url_for('suppliers_page'))