...
from models import Supplier, PurchaseOrder, Product, RecipeComposition, Recipe, Dish, Menu
...
class DataManager:
    def populate_table(self, db, table_name, n):
        """
        Заполняет указанную таблицу тестовыми данными.

        Args:
            db (SQLAlchemy): Объект SQLAlchemy.
        ...
        for _ in range(n):
            new_record = self._generate_fake_data(db, table_class)
            if new_record:
                db.session.add(new_record)
        db.session.commit()
        print(f"{n} records added to table '{table_name}'.")

    def clear_table_data(self, db, table_name, n):
         """
        ...
        """
         ...
         try:
            records_to_delete = table_class.query(db).limit(n).all()
    ...
    def _generate_fake_data(self, db, table_class):
        """
        ...
        if table_class == Supplier:
            ...
            products = Product.query(db).all()
            suppliers = Supplier.query(db).all()
